...
exports.bulkAssignSalary = async (req, res) => {
    ...
    for (const data of salaryData) {
        ...
        try {
            let { employeeId, employeeCode, components = [], dailyWages } = data;
            ...
            // If dailyWages is provided, ensure 60% Basic & 40% Spl components are prepared if missing
            const parsedDailyWages = dailyWages ? parseFloat(dailyWages) : null;
            if (parsedDailyWages && parsedDailyWages > 0) {
                const allComps = await SalaryComponent.findAll({
                    where: { companyId, status: 'Active' },
                    transaction
                });

                const hasBasic = components.some(c => {
                    const found = allComps.find(sc => sc.id === c.componentId);
                ...
                if (!hasBasic) {
                    const basicDef = allComps.find(sc => sc.code === 'BASIC' || sc.name.toLowerCase().includes('basic'));
                    if (basicDef) {
                        components.push({
                            componentId: basicDef.id,
                            valueType: 'Fixed',
                            fixedAmount: parseFloat((parsedDailyWages * 0.6).toFixed(2))
                        });
                    }
                }

                if (!hasSpl) {
                    const splDef = allComps.find(sc => sc.code === 'SPECIAL_ALLOWANCE' || sc.code === 'SPL' || sc.name.toLowerCase().includes('special'));
                    if (splDef) {
                        components.push({
                            componentId: splDef.id,
                            valueType: 'Fixed',
                            fixedAmount: parseFloat((parsedDailyWages * 0.4).toFixed(2))
                        });
                    }
                }
            }
            ...
            if (parsedDailyWages && parsedDailyWages > 0) {
                calculationResult.grossSalary = parsedDailyWages;
                calculationResult.basicSalary = Math.round(parsedDailyWages * 0.6);
                calculationResult.netSalary = calculationResult.grossSalary - calculationResult.totalDeductions;
                calculationResult.ctcMonthly = parsedDailyWages;
                calculationResult.ctcAnnual = parsedDailyWages * 12;

                // Sync daily wage rate with employee record
                await employee.update({ basicSalary: parsedDailyWages }, { transaction });
            }
            ...
            const salaryMaster = await EmployeeSalaryMaster.create({
                ...
                previousSalaryId: null,
                status: 'Active',
                remarks: data.remarks || (parsedDailyWages ? `Daily wage: ₹${parsedDailyWages}/day` : 'Bulk upload'),
                createdBy
            }, { transaction });
...
async function calculateSalaryTotals(components, companyId, employeeId, transaction) {
    ...
    const employee = await Employee.findByPk(employeeId, {
        include: [
            { model: Department, as: 'department', attributes: ['departmentname', ['departmentname', 'name']] },
